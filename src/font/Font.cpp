...
void Font::setup(FT_Face face) {
  ...
  for (unsigned char c = 48; c < CHARACTER_TOTAL; c++) {
    ...
    SpriteRegion region = {{xPos, yPos},
                           {xPos + size.x, size.y},
                           {xPos, yPos + size.y},
                           {xPos + size.x, yPos + size.y}};
    FontCharacter::shared_ptr character =
        std::make_shared<FontCharacter>(_name, c, region, bearing, advance);
...
const std::string Font::toString() const { return _name; }

int Font::atlasWidth() { return ATLAS_SIZE.x; }

int Font::atlasHeight() { return ATLAS_SIZE.y; }

int Font::bytesPerRow() { return ATLAS_SIZE.x; }

std::shared_ptr<ImageFile> Font::generateAtlasImage() {
  return std::make_shared<ImageFile>(atlasWidth(), atlasHeight(), bytesPerRow(),
                                     data());
}
...
LogCategory Font::logCategory() const { return LogCategory::font; }
} // namespace grumble
