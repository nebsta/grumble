...
namespace grumble {
  FileManager::FileManager(FileManagerConfiguration configuration) : _configuration(configuration) {
...
//    logInfo("color_type: " + std::to_string(color_type));
  ...
  void FileManager::writePNG(std::filesystem::path filename, ImageFile::shared_ptr file) {
  ...
  std::filesystem::path FileManager::buildReadPath(std::filesystem::path filename) {
    return _configuration.rootReadPath / filename;
  }

  std::filesystem::path FileManager::buildWritePath(std::filesystem::path filename) {
