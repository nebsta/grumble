...
SpriteManager::SpriteManager(SpriteManagerConfiguration configuration,
    ...
    : _fileManager(fileManager), _configuration(configuration), _allAtlases() {}
...
void SpriteManager::setup() {
  ...
  SpriteManagerConfiguration::AtlasIterator iterator =
      _configuration.atlases.begin();
  for (; iterator != _configuration.atlases.end(); iterator++) {
    ...
    SpriteAtlas::shared_ptr atlas =
        std::make_shared<SpriteAtlas>(atlasName, atlasLayout, atlasImage);
...
std::filesystem::path SpriteManager::buildAtlasImagePath(std::string filename) {
  return _configuration.atlasPath /
         std::filesystem::path(filename + "-sheet.png");
}

std::filesystem::path SpriteManager::buildAtlasDataPath(std::string filename) {
  return _configuration.atlasPath /
         std::filesystem::path(filename + "-data.json");
}
