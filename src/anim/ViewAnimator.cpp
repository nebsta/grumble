...
ViewAnimator::ViewAnimator(glm::vec2 startPosition, glm::vec2 startSize)
    : _startPosition(startPosition), _startSize(startSize),
      _endPosition(VECTOR_NEGATIVE), _endSize(VECTOR_NEGATIVE),
      _resultPosition(VECTOR_EMPTY), _resultSize(VECTOR_EMPTY),
      _currentPosition(VECTOR_EMPTY), _currentSize(VECTOR_EMPTY) {}

ViewAnimator::ViewAnimator()
    : ViewAnimator(VIEW_ANIM_DEFAULT_START_POS, VIEW_ANIM_DEFAULT_START_SIZE) {}
...
glm::vec2 ViewAnimator::resultPosition() const { return _resultPosition; }

glm::vec2 ViewAnimator::resultSize() const { return _resultSize; }

glm::vec2 ViewAnimator::startSize() const { return _startSize; }

glm::vec2 ViewAnimator::endSize() const { return _endSize; }

glm::vec2 ViewAnimator::startPosition() const { return _startPosition; }

glm::vec2 ViewAnimator::endPosition() const { return _endPosition; }
} // namespace grumble
